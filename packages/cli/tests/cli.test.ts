...
import { buildScaffold } from '../src/templates.js'
...
describe('buildScaffold', () => {
  it('creates minimal template files', () => {
    const plan = buildScaffold('demo', 'minimal', '0.0.1')
    expect(plan.files['src/app.jcr']).toContain('signal')
    expect(plan.files['package.json']).toContain('"@jacare/cli": "0.0.1"')
    expect(plan.assets).toHaveLength(0)
  })

  it('creates nav template with pages', () => {
    const plan = buildScaffold('demo', 'nav', '0.0.1')
  ...
  it('creates todo template with devtools', () => {
    const plan = buildScaffold('demo', 'todo', '0.0.1')
